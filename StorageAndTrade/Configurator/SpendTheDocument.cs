...
";
		...
		public static void ДокументНеПроводиться(UnigueID Обєкт, List<string> СписокПомилок)
        {
			ФункціїДляПовідомлень.ДодатиПовідомленняПроПомилку(DateTime.Now, Обєкт.UGuid, string.Join("\n", СписокПомилок.ToArray()));
		}
	}
		...
		public static bool Spend(ЗамовленняКлієнта_Objest ДокументОбєкт)
		{
			...
			foreach (ЗамовленняКлієнта_Товари_TablePart.Record ТовариРядок in ДокументОбєкт.Товари_TablePart.Records)
			{
				if (ТовариРядок.Номенклатура.IsEmpty())
				{
					...
					СпільніФункції.ДокументНеПроводиться(ДокументОбєкт.UnigueID, СписокПомилок);
